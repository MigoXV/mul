...
class ResNet18(BaseFairseqModel):
    ...
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(  # layer1有点特别，由于输入输出的channel均是64，故两个CommonBlock
            CommonBlock(64, 64, 1), CommonBlock(64, 64, 1)
        )
        self.layer2 = nn.Sequential(  # layer234类似，由于输入输出的channel不同，故一个SpecialBlock，一个CommonBlock
            SpecialBlock(64, 128, [2, 1]), CommonBlock(128, 128, 1)
        )
        self.layer3 = nn.Sequential(
            SpecialBlock(128, 256, [2, 1]), CommonBlock(256, 256, 1)
        )
        self.layer4 = nn.Sequential(
            SpecialBlock(256, 512, [2, 1]), CommonBlock(512, 512, 1)
        )
        ...
        self.fc = nn.Sequential(  # 最后用于分类的全连接层，根据需要灵活变化
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(p=0.5),
            nn.Linear(256, 1),  # 这个使用CIFAR10数据集，定为10分类
        )
    ...
    def forward(self, x):
        ...
        x = self.fc(x).squeeze(-1)  # 全连接层

        return x


@register_model_architecture("resnet18", "resnet18-bin")
def vad_bert_sample_arch(args):
    pass
